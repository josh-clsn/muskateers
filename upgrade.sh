...
#
# It also monitors system-wide RAM usage after each node update.
# If RAM usage hits >= 85%, the script immediately terminates.
# Now it prints RAM usage each time it checks.
#
...
#############################
check_ram() {
    # Pull total vs used memory from the 'free' command
    # The 'Mem:' line is typically:  Mem:  total used free ...
...
echo "============================================="
echo " antNode Updater with RAM Monitoring "
echo "============================================="
...
for service_path in "$services_dir"/antnode*; do
    # Check RAM before proceeding with next node
    check_ram
    ...
    # Check RAM usage again at the end of iteration (optional):
    check_ram
