...
# located under /var/antctl/services/antnode*, without using a GitHub
...
# The personal access token for GitHub (previously used for private repos)
...
log "====================="
log "Starting antnode update script (no token)."
log "====================="
...
# Make sure we have a GitHub token -- NOT needed anymore, so comment out:
# if [ -z "$github_token" ]; then
#     log "Error: No GitHub token provided. Please set the GITHUB_TOKEN environment variable."
#     exit 1
# fi

# Download the new binary -- remove the Authorization header:
...
if ! wget -q -O "$new_binary" "$new_binary_url"; then
    log "Error: Failed to download the antnode binary (check URL)."
    exit 1
fi
