...
#   1) Update all nodes (running or not)
#   2) Only update nodes that are NOT running
#
...
for service_path in "$services_dir"/antnode*; do
    ...
    if systemctl is-active --quiet "$service_name"; then
        ...
        if [ "$update_choice" = "2" ]; then
            # If the user chose "ONLY update nodes that are NOT running", skip this one
    ...
    # Start the service if it was running OR if it was not running (and we choose to always start)
    # Because your requirement says: "sometimes I only want to start the binaries on nodes
    # that are NOT yet running..." => we interpret that if it's not running, we definitely start it.
    # If it was running, we also start it again (since we just updated it).
